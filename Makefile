...
# Licensed under the MIT License, ⓒ Daniel Wambua 2024 <danielwambua.com>       #
################################################################################

# Specify that the targets are not files
.PHONY: all install validate generate compile markdown clean clean_tex watch web_install web web_dev
...
OUTPUT_PDF := web/static/downloads/Daniel-Wambua-CV.pdf
OUTPUT_MD := web/generated/Daniel-Wambua-CV.md

# Default target. Install deps, validate, generate, compile, and emit markdown.
# PDF now writes directly to the web app static downloads directory.
